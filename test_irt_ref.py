...
if __name__=="__main__":
    ...
    # Select reference legislators based on DW-NOMINATE scores
    # For dimension 1 (liberal-conservative): most liberal and most conservative
    most_liberal_idx = member_info_df['nominate_dim_1'].idxmin()
    most_conservative_idx = member_info_df['nominate_dim_1'].idxmax()

    # # For dimension 2: legislators with extreme scores on dimension 2
    # # but moderate on dimension 1 (to avoid collinearity)
    # dim2_candidates = member_info_df[
    #     (member_info_df['nominate_dim_1'] > -0.5) & 
    #     (member_info_df['nominate_dim_1'] < 0.5)
    # ].copy()

    # if len(dim2_candidates) > 0:
    #     most_negative_dim2_idx = dim2_candidates['nominate_dim_2'].idxmin()
    #     most_positive_dim2_idx = dim2_candidates['nominate_dim_2'].idxmax()
    # else:
    #     # Fallback: use overall extremes on dimension 2
    #     most_negative_dim2_idx = member_info_df['nominate_dim_2'].idxmin()
    #     most_positive_dim2_idx = member_info_df['nominate_dim_2'].idxmax()

    print("Reference legislators:")
    print(f"Most liberal (dim1): {member_info_df.loc[most_liberal_idx, 'bioname']} "
        f"(NOMINATE: {member_info_df.loc[most_liberal_idx, 'nominate_dim_1']:.3f})")
    print(f"Most conservative (dim1): {member_info_df.loc[most_conservative_idx, 'bioname']} "
        f"(NOMINATE: {member_info_df.loc[most_conservative_idx, 'nominate_dim_1']:.3f})")
    # print(f"Most negative dim2: {member_info_df.loc[most_negative_dim2_idx, 'bioname']} "
    #       f"(NOMINATE dim2: {member_info_df.loc[most_negative_dim2_idx, 'nominate_dim_2']:.3f})")
    # print(f"Most positive dim2: {member_info_df.loc[most_positive_dim2_idx, 'bioname']} "
    #       f"(NOMINATE dim2: {member_info_df.loc[most_positive_dim2_idx, 'nominate_dim_2']:.3f})")

    # UPDATE: For the 3rd legislator: choose someone orthogonal to the lib-con dimension
    # Find legislator with extreme dim2 score but moderate dim1 score
    moderate_dim1 = member_info_df[
        (member_info_df['nominate_dim_1'] > -0.3) & 
        (member_info_df['nominate_dim_1'] < 0.3)
    ].copy()

    if len(moderate_dim1) > 0:
        third_ref_idx = moderate_dim1['nominate_dim_2'].abs().idxmax()  # Most extreme on dim2
    else:
        third_ref_idx = member_info_df['nominate_dim_2'].abs().idxmax()

    print(f"Most orthogonal dim2: {member_info_df.loc[third_ref_idx, 'bioname']} "
        f"(NOMINATE dim2: {member_info_df.loc[third_ref_idx, 'nominate_dim_2']:.3f})")
    #-------------------------------- Fix extreme Legislators --------------------------------#
    ...
    stan_data = {
        ...
        'y': vote_obs,
    #-------------------------------- Fix extreme Legislators --------------------------------#
        'ref_j1': most_liberal_idx + 1,      # Most liberal (will be negative on dim1)
        'ref_j2': most_conservative_idx + 1,  # Most conservative (will be positive on dim1)
        # 'ref_j3': most_negative_dim2_idx + 1, # Extreme negative on dim2
        # 'ref_j4': most_positive_dim2_idx + 1  # Extreme positive on dim2
        'ref_j3': third_ref_idx + 1          # Fix at (0, ±2)
    #-------------------------------- Fix extreme Legislators --------------------------------#
    }
    ...
    for idx in range(10):
        ...
        plot_ideal_points(ideal_points, f"3_ref_leg_{idx}")
