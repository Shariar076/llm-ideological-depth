...
for data_name in ${data_names[@]}; do
    for eval_data_name in politically-liberal; do
        for ((i=0; i<${layer_num}; i++)); do
            layer=${layers[$i]}
            log_path=./results/${data_name}/${model_name}_results_${mode}/logs/main/caa/eval_${eval_data_name}/${model_name}_steer_${data_name}_caa_${arg_type}_layer${layer}.result.log
            ...
            output_file=./results/${data_name}/${model_name}_results_${mode}/main/caa/eval_${eval_data_name}/${model_name}_steer_${data_name}_caa_${arg_type}_layer${layer}.result.json
