...
if __name__ == "__main__":
    ...
    parser.add_argument("--mymultis", nargs="+", type=float, required=True)
    ...
    parser.add_argument("--arg_type", type=str, default=None)
    ...
    #     )
    model = LlamaWrapper(args.model_name_or_path) if "llama" in args.model_name else GemmaWrapper(args.model_name_or_path)
    ...
    for i in range(len(test_dataset)):
        ...
        if args.model_name=="gemma-2-9b-it" and args.AB==False:
            if args.system_prompt != "":
                if ques is not None:
                    ques = args.system_prompt + " " + ques
                else:
            ...
            prompt_tokens_list.append(tokenizer.encode(ques, return_tensors="pt").to(device))
        elif args.model_name=="llama-3.1-8b-it" and args.AB==False:
    ...
    for layer in args.layers:
        for mymulti in args.mymultis:
            ...
            output_file = args.output_file.replace(".result.json", f"_mymulti={mymulti}.result.json")
