...
if __name__ == "__main__":
    ...
    for i in range(len(vector_dataset)):
        ...
        if ques and chosen and rejected:
            ...
            neg_tokens = tokenizer.encode(ques + f"({rejected})", return_tensors="pt")
    ...
    if args.model_name in ["gemma-2-9b-it", "llama-3-8b-it", "llama-3.1-8b-it"]:
    ...
    for layer in args.layers:
        ...
        # torch.save(
        #     all_pos_layer,
        #     os.path.join(output_dir, f"{layer}-pos.pt"),
        # )
        # torch.save(
        #     all_neg_layer,
        #     os.path.join(output_dir, f"{layer}-neg.pt"),
        # )
        torch.save(
            vec,
